...
var ContentFlowGlobal = {
    ...
    init: function () {
        ...
        if (document.addEventListener) {
            ...
            document.write("<script id=__ie_cf_onload defer src=\"://\"><\/script>");
...
var ContentFlowItem  = function (CFobj, element, index) {
    ...
    if (this.content && this.content.nodeName == "IMG") {
        ...
        if (this.content.complete && this.content.width > 0)
            ...
            this.content.onload = function () { window.setTimeout(foobar, 100); }
    }
    else {
        this.initClick();
        CFobj._addItemCueProcess(true);
...
ContentFlow.prototype = {
    ...
    _defaultConf: {
        ...
        biggestItemPos: 0,
        loadingTimeout: 120000, //milliseconds
        ...
        calcStepWidth: function(diff,speedup) {
...
//            }
            ...
            stepwidth *= Math.abs(speedup);

            if( diff >= 0 ) {
                return Math.min(stepwidth,Math.max(diff,0.08));
            } else {
                return Math.max(stepwidth,Math.min(diff,-0.08));
            }
        },
    ...
    _init: function () {
        ...
        var timer = window.setInterval (
            function() {
                if ( cf._imagesToLoad == 0 || ((new Date().getTime())) - now > cf.loadingTimeout ) {
                    clearInterval(timer);
    ...
    _step: function () {
        ...
        if ( absDiff > 0.001) { // till activeItem is nearly at position 0
            ...
            if( this._lastStepTime > 0 ) {
                var timeDelta = currentTime - this._lastStepTime;
                speedup = timeDelta / this._millisecondsPerStep;
                this._lastStepTime = currentTime;
            } else {
                speedup = this.conf.flowSpeedFactor;
                this._lastStepTime = currentTime;
            }
            this._currentPosition += this.conf.calcStepWidth(diff, Math.min(3,speedup));
